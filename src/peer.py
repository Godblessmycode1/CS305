...
MAX_PAYLOAD = 1024
#sending_now是个list 储存正在向哪些peer发送。
sending_now=[]
#requiring_now是个dict,储存当前的download_hash由哪个peer发送，防止好多个peer传给自己相同的chunk_data。
requiring_now=dict()
config = None
ex_output_file = None
...
def process_inbound_udp(sock):
    ...
    global sending_now
    global requiring_now
    pkt, from_addr = sock.recvfrom(BUF_SIZE)
    Magic, Team, Type,hlen, plen, Seq, Ack= struct.unpack("HBBHHII", pkt[:HEADER_LEN])
    ##这里的data数据要根据plen来判断读多少,是payload
    data = pkt[HEADER_LEN:]
    if Type == 1:
        ...
        # received a DATA pkt
        if  ex_downloading_chunkhash not in requiring_now.keys(): #第一次收到某个peer的包
             requiring_now[ex_downloading_chunkhash]=from_addr
             ex_received_chunk[ex_downloading_chunkhash] += data
             # send back ACK
             ack_pkt = struct.pack("HBBHHII", socket.htons(52305),35,  4,socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), 0,Seq)
             sock.sendto(ack_pkt, from_addr)
        elif ex_downloading_chunkhash in requiring_now.keys() and from_addr!=requiring_now[ex_downloading_chunkhash]: #别的peer也发来,发回去denied消息
             # send back denied
             denied_pkt = struct.pack("HBBHHII", socket.htons(52305),35, 5, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), socket.htonl(0), 0)
             sock.sendto(denied_pkt,from_addr)
        elif ex_downloading_chunkhash in requiring_now.keys() and from_addr==requiring_now[ex_downloading_chunkhash]:
             ex_received_chunk[ex_downloading_chunkhash] += data
             # send back ACK
             ack_pkt = struct.pack("HBBHHII", socket.htons(52305),35,  4,socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), 0,Seq)
             sock.sendto(ack_pkt, from_addr)
        ...
        #没有达到max_send
        if (from_addr not in sending_now) and len(sending_now)<config.max_conn:
           # send back DATA
           ...
           #端口加一
           sending_now.append(from_addr)
        else :
           #达到max_send 开始发送denied 信息。
           denied_pkt = struct.pack("HBBHHII", socket.htons(52305),35, 5, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), socket.htonl(0), 0)
           sock.sendto(denied_pkt,from_addr)
